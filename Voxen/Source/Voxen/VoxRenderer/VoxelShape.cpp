...
namespace Voxen
{
    ...
    void Voxen::VoxelShape::LoadDefaultPalette()
    {
		// Palette from .vox file type
		constexpr uint32 defaultPalette[255] =
		{
			0xffffff, 0x000000, 0xccffff, 0x99ffff, 0x66ffff, 0x33ffff, 0x00ffff, 0xffccff,
			...
			0xbbbbbb, 0xaaaaaa, 0x888888, 0x777777, 0x555555, 0x444444, 0x222222
		};
		...
		{
			uint32 color = defaultPalette[i];

			uint8 red   = (color >> 16) & 0xFF;
			uint8 green = (color >>  8) & 0xFF;
			uint8 blue  = (color >>  0) & 0xFF;
			SetMaterial(i, VoxelMaterial(red, green, blue, 0, 0, 0, 0));
		}
    }
