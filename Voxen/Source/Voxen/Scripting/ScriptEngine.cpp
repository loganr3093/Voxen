...
namespace Voxen
{
	static std::unordered_map<std::string, ScriptFieldType> s_ScriptFieldTypeMap =
	{
		...
		{ "Voxen.Vector2", ScriptFieldType::Vector2 },
		{ "Voxen.Vector3", ScriptFieldType::Vector3 },
		{ "Voxen.Vector4", ScriptFieldType::Vector4 },

		{ "Voxen.Entity", ScriptFieldType::Entity },
	};
	...
	namespace Utils
	{
		...
		ScriptFieldType MonoTypeToScriptFieldType(MonoType* monoType)
		{
			...
			{
				VOX_CORE_ERROR("Unknown type: {}", typeName);
	...
	void ScriptEngine::LoadAssemblyClasses()
	{
		...
		for (int32 i = 0; i < numTypes; i++)
		{
			...
			VOX_CORE_WARN("{0} fields:", className);
			...
			while (MonoClassField* field = mono_class_get_fields(monoClass, &iterator))
			{
				...
				if (flags & FIELD_ATTRIBUTE_PUBLIC)
				{
					...
					VOX_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
	...
	{
		VOX_CORE_ASSERT(s_Data->EntityInstances.find(uuid) != s_Data->EntityInstances.end());
